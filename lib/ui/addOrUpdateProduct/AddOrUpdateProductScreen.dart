...
import 'package:foodie_restaurant/ui/addOrUpdateProduct/add_option.dart';
...
class _AddOrUpdateProductScreenState extends State<AddOrUpdateProductScreen> {
                        ...
                        child: Column(
                          ...
                          children: [
                                        ...
                                        showDialog(
                                          ...
                                          builder: (BuildContext context) =>
                                              AddCategoryView(
                                                  ...
                                                  max: "",
                                                  selectedCatType: "",
                                                onDone: (catName, catType, chooseMax) {
                                                  final timestamp = DateTime.now().millisecondsSinceEpoch.toRadixString(36);
                                                  final randomStr = Random().nextInt(100000000).toRadixString(36);
                                                  final truncatedRandomStr = randomStr.substring(0, min(7, randomStr.length));
                                                  final uniqueId = timestamp + truncatedRandomStr;
                                                  final bytes = utf8.encode(uniqueId);
                                                  final catId = base64.encode(bytes).substring(0, 12);

                                                  Categories cat = Categories(catName, catType, proId, catId, "", chooseMax);
                            ...
                            ListView.builder(
                              ...
                              physics: NeverScrollableScrollPhysics(),
                              itemBuilder: (context, index) {
                                Categories cat = categoriesList[index];
                                print("catName: " + categoriesList[index].categoryName.toString());
                                return Container(
                                  ...
                                  child: Column(
                                    children: [
                                      Row(
                                        children: [
                                          Expanded(
                                            flex: 3,
                                            child: TextField(
                                              controller: TextEditingController(text: categoriesList[index].categoryName),
                                              readOnly: true,
                                              decoration: InputDecoration(
                                                  ...
                                                  showDialog(
                                                    ...
                                                    builder: (BuildContext context) =>
                                                        AddOptionView(
                                                            ...
                                                            description: "",
                                                            optionDefault: false,
                                                            onDone: (optionDefault, optName, optPrice, des) {
                                                              final timestamp = DateTime.now().millisecondsSinceEpoch.toRadixString(36);
                                                              final randomStr = Random().nextInt(100000000).toRadixString(36);
                                                              final truncatedRandomStr = randomStr.substring(0, min(7, randomStr.length));
                                                              final uniqueId = timestamp + truncatedRandomStr;
                                                              final bytes = utf8.encode(uniqueId);
                                                              final opt_id = base64.encode(bytes).substring(0, 12);
                                                              ...
                                                              cat.optionsList.add(optionsModel);
                                                              setState(() {});
                                                            }
                                                        ),
                                                  );
                                                  ...
                                                  showDialog(
                                                    ...
                                                    builder: (BuildContext context) =>
                                                        AddCategoryView(
                                                          ...
                                                          max: categoriesList[index].chooseMax,
                                                          selectedCatType: categoriesList[index].categoryType,
                                                            onDone: (catName, catType, chooseMax) {
                                                              Categories cat = Categories(catName, catType, proId, categoriesList[index].id, "", chooseMax);
                                                              // categoriesList.add(cat);
                                                              categoriesList[index] = cat;
                                                              setState(() {});
                                                            }
                                                        ),
                                                  );
                                      ...
                                      ListView.builder(
                                        itemCount: cat.optionsList.length,
                                        shrinkWrap: true,
                                        physics: NeverScrollableScrollPhysics(),
                                        itemBuilder: (context, index) {
                                          print("optName: " + cat.optionsList[index].toString());
                                          return Container(
                                            margin: EdgeInsets.all(8.0),
                                            padding: EdgeInsets.all(8.0),
                                            decoration: BoxDecoration(
                                                ...
                                                Row(
                                                  children: [
                                                    Expanded(
                                                      flex: 3,
                                                      child: TextField(
                                                        controller: TextEditingController(text: cat.optionsList[index].optionName),
                                                        readOnly: true,
                                                        decoration: InputDecoration(
                                                          hintText: 'Category Name',
                                                        ),
                                                    ...
                                                    Expanded(
                                                      flex: 2,
                                                      child: TextField(
                                                        controller: TextEditingController(text: cat.optionsList[index].optionPrice),
                                                        readOnly: true,
                                                      ),
                                                    ),
                                                    SizedBox(width: 10),
                                                    Expanded(
                                                      flex: 2,
                                                      child: TextField(
                                                        controller: TextEditingController(text: cat.optionsList[index].description),
                                                        readOnly: true,
                                                      ),
                                                    ),
                                                  ],
                                                  ...
                                                  children: [
                                                    Expanded(
                                                      ...
                                                      child: Row(
                                                        // mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                                        children: [
                                                          Checkbox(value: cat.optionsList[index].optionDefault, activeColor: Color(COLOR_PRIMARY), onChanged: (value) {}),
                                                          Text(
                                                            "Default",
                                                    ...
                                                    Expanded(
                                                      ...
                                                      child: ElevatedButton(
                                                        onPressed: () {
                                                          cat.optionsList.removeAt(index);
                                                          setState(() {});
                                                        },
                                                        child: Icon(Icons.delete),
                                                      ),
                                                        ...
                                                        onPressed: () {
                                                          setState(() {
                                                            showDialog(
                                                              ...
                                                              builder: (BuildContext context) =>
                                                                  AddOptionView(
                                                                      title: cat.optionsList[index].optionName,
                                                                      max: cat.optionsList[index].optionPrice,
                                                                      description: cat.optionsList[index].description,
                                                                      optionDefault: cat.optionsList[index].optionDefault,
                                                                      onDone: (optionDefault, optName, optPrice, des) {
                                                                        Options optionsModel = Options(
                                                                            optionDefault,
                                                                            optName,
                                                                            optPrice,
                                                                            des,
                                                                            cat.optionsList[index].id,
                                                                            cat.id
                                                                        );
                                                                        // cat.optionsList.add(optionsModel);
                                                                        cat.optionsList[index] = optionsModel;
                                                                        setState(() {});
                                                                      }
                                                                  ),
                                                            );
  ...
  String uniqueId({String prefix = "", bool random = false}) {
    final sec = DateTime.now().microsecondsSinceEpoch + Random().nextInt(1000); // Using DateTime.now() to get current time in microseconds
    final id = sec.toRadixString(16).replaceAll('.', "").padRight(14, "0");
    return '$prefix$id${random ? '.${Random().nextInt(100000000)}' : ""}';
  }

  getList() async {
    categoriesList.forEach((element) async {
      ...
      optList.addAll(element.optionsList);
    });
  }
  ...
  submit() async {
    if (veg == false && nonVeg == false) {
      ...
      if (_key.currentState?.validate() ?? false) {
        ...
        List<Map<String, dynamic>> categoryMaps = catList.map((category) {
          return {
            'id': category.id,
            'category_name': category.categoryName,
            'category_type': category.categoryType,
            'product_id': category.productId,
            'choose_max': category.chooseMax,
          };
        }).toList();

        List<Map<String, dynamic>> optionsMaps = optList.map((option) {
          return {
            'id': option.id,
            'option_default': option.optionDefault,
        ...
        productModel.categoriesMaps = categoryMaps;
        productModel.optionsMaps = optionsMaps;
        await fireStoreUtils.addCategory(catList);
        await fireStoreUtils.addCategoryOption(optList);
