...
class ScheduleOrdersScreen extends StatefulWidget {
  @override
  _ScheduleOrdersScreenState createState() => _ScheduleOrdersScreenState();
}

class _ScheduleOrdersScreenState extends State<ScheduleOrdersScreen> {
  ...
  void initState() {
    super.initState();
    setCurrency();
    ordersStream = _fireStoreUtils.watchScheduleOrdersStatus(MyAppState.currentUser!.vendorID);
            ...
            child: StreamBuilder<List<OrderModel>>(
                ...
                builder: (context, snapshot) {
                  ...
                  if (!snapshot.hasData || (snapshot.data?.isEmpty ?? true)) {
                    return Center(
                      child: showEmptyState('No Scheduled Orders'.tr(), 'New scheduled order requests will show up here'.tr()),
                    );
                  } else {
            ...
            children: [
              Container(
                  ...
                  child: Row(children: [
                    ...
                    Expanded(
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
                        ...
                        Text(
                          ...
                          style: TextStyle(fontSize: 15, color: isDarkMode(context) ? Colors.white : Color(0XFF000000), letterSpacing: 0.5, fontFamily: 'Poppinsm'),
                        ),
                        SizedBox(
                          height: 6,
                        ),
                        orderModel.takeAway!
                            ? Text(
                                'Takeaway'.tr(),
                                style: TextStyle(fontSize: 13, color: isDarkMode(context) ? Colors.white : Color(0XFF555353), letterSpacing: 0.5, fontFamily: 'Poppinsl'),
                              )
                            : Row(children: [
                        ...
                        orderModel.takeAway!
                            ...
                            : Container(
                                ...
                                child: Text(
                                  ...
                                  maxLines: 2,
                                  style: TextStyle(color: isDarkMode(context) ? Colors.white : Color(0XFF555353), fontSize: 13, letterSpacing: 0.5, fontFamily: 'Poppinsr'),
                                ),
                              ),
                      ]),
                    ),
                    Column(
                      children: [
                        Text(
                          DateFormat("EEEE").format(DateTime.parse(orderModel.scheduleTime!)).toString(),
                          style: TextStyle(fontSize: 12, color: isDarkMode(context) ? Colors.white : Color(0XFF555353), letterSpacing: 0.5, fontFamily: 'Poppinsm'),
                        ),
                        Text(
                          DateFormat("h:mm: a").format(DateTime.parse(orderModel.scheduleTime!)).toString(),
                          style: TextStyle(fontSize: 12, color: isDarkMode(context) ? Colors.white : Color(0XFF555353), letterSpacing: 0.5, fontFamily: 'Poppinsm'),
                        ),
                      ],
                    ),
              ...
              ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    print("cartProduct.productCategories: " + orderModel.orderProduct[index].productCategories.toString());

                    if (orderModel.orderProduct[index].productCategories != '') {
                      categories = Categories.decode(orderModel.orderProduct[index].productCategories);
                      categories.forEach((element) {
                        if (element.options!.isNotEmpty) {
                          element.optionsList = Options.decode(element.options!);
                        }
                      });
                    }
                    ...
                    String amount = "0.00";
                    if (product.extrasPrice!.isNotEmpty && double.parse(product.extrasPrice!) != 0.0) {
                        ...
                        categories.isEmpty || categories.length == 0
                            ...
                            child: ListView.builder(
                                ...
                                itemBuilder: (BuildContext context, int index) {
                                  String listString = "";
                                  List<String> lstOpt = [];
                                  categories![index].optionsList.forEach((element) {
                                  ...
                                  return listString.isNotEmpty
                                    ...
                                    children: [
                                      Text(
                                        ...
                                        style: TextStyle(fontFamily: 'Poppinsm', color: isDarkMode(context) ? Colors.white : Color(0XFF2A2A2A), fontSize: 14),
                                      ),
                                      Flexible(
                                        child: Text(
                                          listString,
                                          style: TextStyle(fontFamily: 'Poppinssl', color: isDarkMode(context) ? Colors.white : Color(0XFF2A2A2A), fontSize: 14),
              ...
              Container(
                  ...
                  child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                    ...
                    Text(
                      symbol != '' ? symbol + totalAmount : '$symbol$totalAmount',
                        ...
                        Expanded(
                          child: ElevatedButton(
                            ...
                            onPressed: () {
                              ...
                              FireStoreUtils.sendFcmMessage("Order Confirmed".tr(), "Your Order Has Been Confirmed", orderModel.author!.fcmToken, orderModel.id);

                              if (orderModel.status == ORDER_STATUS_PLACED && !orderModel.takeAway!) {
                                FireStoreUtils.sendFcmMessage("New Delivery!".tr(), 'New Delivery Request'.tr(), orderModel.driver!.fcmToken, orderModel.id);
                              }
                              setState(() {});
                            },
                            child: Text(
                        ...
                        Expanded(
                          child: ElevatedButton(
                            ...
                            onPressed: () {
                              ...
                              FireStoreUtils.updateOrder(orderModel);
                              FireStoreUtils.sendFcmMessage("Order Rejected".tr(), "Your Order Has Been Cancelled", orderModel.author!.fcmToken, orderModel.id);
                              ...
                              if (orderModel.paymentMethod == "stripe") {
                                if (orderModel.paymentID != null || orderModel.paymentID != "") {
                                  cancelFunds(orderModel.paymentID!);
                                }
                              }

                              if (orderModel.status == ORDER_STATUS_REJECTED && !orderModel.takeAway!) {
                                FireStoreUtils.sendFcmMessage("Reject Order!".tr(), 'Reject Order Request'.tr(), orderModel.driver!.fcmToken, orderModel.id);
                              }
                              setState(() {});
                            },
                            child: Text(
                          ...
                          ? Expanded(
                              child: GestureDetector(
                                onTap: () {
                                  ...
                                  FireStoreUtils.sendFcmMessage("Order Delivered", "Your Order Has Been Delivered", orderModel.author!.fcmToken, orderModel.id);
                                },
                                child: Container(
