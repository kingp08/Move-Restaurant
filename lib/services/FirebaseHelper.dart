...
import 'package:foodie_restaurant/model/CategoryOption.dart';
import 'package:foodie_restaurant/model/ChannelParticipation.dart';
...
import 'package:foodie_restaurant/model/ProductCategories.dart';
...
class FireStoreUtils {
  ...
  static Future addChat(ConversationModel conversationModel) async {
    return await firestore.collection("chat_restaurant").doc(conversationModel.orderId).collection("thread").doc(conversationModel.id).set(
        conversationModel.toJson()).then((document) {
  ...
  Future<RatingModel?> getOrderReviewsbyID(String ordertId, String productId) async {
    ...
    QuerySnapshot<Map<String, dynamic>> vendorsQuery = await firestore.collection(Order_Rating).where('orderid', isEqualTo: ordertId).where(
  ...
  static Future<TableModel?> addTable(TableModel bookTableModel, VendorModel vendorModel) async {
    try {
      await firestore.collection(VENDORS).doc(vendorModel.id).collection(CREATETABLE).doc(bookTableModel.tableId).set(
  ...
  static Future<bool> sendFcmMessage(String title, String message, String token, String orderId, {String? type}) async {
    print(token.toString() + "====================TOKEN");
    DateTime futureTime = DateTime.now().add(Duration(minutes: 5));
    String timestampToSend = futureTime.toIso8601String();
    try {
  ...
  Future<bool> blockUser(User blockedUser, String type) async {
    ...
    BlockUserModel blockUserModel = BlockUserModel(
        type: type, source: MyAppState.currentUser!.userID, dest: blockedUser.userID, createdAt: Timestamp.now());
...
// if (docSnapshot.exists) {
  ...
  Future<List<OrderModel>> getVendorOrders(String userID) async {
    ...
    QuerySnapshot<Map<String, dynamic>> ordersQuery = await firestore.collection(ORDERS).where('vendorID', isEqualTo: userID).orderBy(
  ...
  Stream<List<OrderModel>> watchOrdersStatus(String vendorID) async* {
    ...
    ordersStreamSub =
        firestore.collection(ORDERS).where('vendorID', isEqualTo: vendorID).orderBy('createdAt', descending: true).limit(20).snapshots().listen((
            event) async {
          orders.clear();
          await Future.forEach(event.docs, (QueryDocumentSnapshot<Map<String, dynamic>> element) {
            try {
              ...
              //   if (orderModel.additional.isNotEmpty) {
              //     double percentage = ((double.parse(orderModel.additional.toString()) / 100) * double.parse(element.price.toString()));
              //     element.price = (double.parse(element.price.toString()) - percentage).toString();
              //   }
              // });
              if (!orderModel.scheduleStatus!) {
  ...
  Stream<List<OrderModel>> watchScheduleOrdersStatus(String vendorID) async* {
    print(vendorID.toString() + "====123");
    List<OrderModel> orders = [];
    ordersStreamController = StreamController.broadcast();
    ordersStreamSub =
        firestore.collection(ORDERS).where('vendorID', isEqualTo: vendorID).orderBy('createdAt', descending: true).snapshots().listen((
            event) async {
          orders.clear();
          await Future.forEach(event.docs, (QueryDocumentSnapshot<Map<String, dynamic>> element) {
            try {
              OrderModel orderModel = OrderModel.fromJson(element.data());
              print(orderModel.scheduleTime.toString() + "+++====123456+++");
              orderModel.orderProduct.forEach((element) {
                if (orderModel.additional.isNotEmpty) {
                  element.price = (double.parse(element.price.toString()) - double.parse(orderModel.additional.toString())).toString();
                }
              });
              if (orderModel.scheduleTime != null && orderModel.scheduleTime!.isNotEmpty && orderModel.scheduleStatus!) {
                // if (orderModel.scheduleTime!.isBefore(DateTime.now())) {
                //
                // }
                orders.add(orderModel);
              }
  ...
  static Future updateScheduleOrderStatus(String vendorID) async {
    firestore.collection(ORDERS).where('vendorID', isEqualTo: vendorID).orderBy('createdAt', descending: true).snapshots().listen((
        event) async {
      await Future.forEach(event.docs, (QueryDocumentSnapshot<Map<String, dynamic>> element) async {
        try {
          OrderModel orderModel = OrderModel.fromJson(element.data());
          if (orderModel.scheduleTime != null && orderModel.scheduleTime!.isNotEmpty && orderModel.scheduleStatus!) {
            DateTime now = DateTime.now();
            DateTime oneHourLater = now.add(Duration(hours: 1));
            DateTime givenDateTime = DateTime.parse(orderModel.scheduleTime!);
            if (givenDateTime.isAfter(now) && givenDateTime.isBefore(oneHourLater)) {
              await firestore.collection(ORDERS).doc(orderModel.id).update({'scheduleStatus': false}).then((value) => {
                display("Order Time", "It’s time to prepare an order that was scheduled.")
              });
            } else {
              if (givenDateTime.isBefore(oneHourLater)) {
                await firestore.collection(ORDERS).doc(orderModel.id).update({'scheduleStatus': false});
              }
              print("order is not scheduled");
            }
          }
        } catch (e, s) {
          print('watchOrdersStatus parse error ${element.id}$e $s');
  ...
  addOrUpdateProduct(ProductModel productModel) async {
    ...
    if ((productModel.id).isNotEmpty) {
      ...
      // firestore.collection("product_categories").where('product_id', isEqualTo: productModel.id).snapshots().listen((value) async {
      //   await Future.forEach(value.docs, (QueryDocumentSnapshot<Map<String, dynamic>> element) async {
      //     try {
      //
      //       Categories cat = Categories.fromJson(element.data());
      //       categories.add(cat);
      //
      //       firestore.collection("category_option").where('category_id', isEqualTo: cat.id).snapshots().listen((value) async {
      //         await Future.forEach(value.docs, (QueryDocumentSnapshot<Map<String, dynamic>> element) {
      //           try {
      //             options.add(Options.fromJson(element.data()));
      //           } catch (e, s) {
      //             print('getProductsStream parse error ${element.id}$e $s');
      //           }
      //         });
  ...
  addCategory(List<CategoryModel> catList) async {
    catList.forEach((element) async {
      await firestore.collection(STORY).doc("product_categories").set(element.toJson());
    });
  }

  addCategoryOption(List<Options> optList) async {
    optList.forEach((element) async {
      await firestore.collection(STORY).doc("category_option").set(element.toJson());
    });
  }
...
// or FacebookAuth.i.permissions
  ...
  static handleFacebookLogin(Map<String, dynamic> userData, AccessToken token) async {
    // print(token);
    auth.UserCredential authResult = await auth.FirebaseAuth.instance.signInWithCredential(
  ...
  Future<File> _compressVideo(File file) async {
    MediaInfo? info = await VideoCompress.compressVideo(
        file.path, quality: VideoQuality.DefaultQuality, deleteOrigin: false, includeAudio: true, frameRate: 24);
    if (info != null) {
  ...
  static firebaseSignUpWithEmailAndPassword(String emailAddress, String password, File? image, String firstName, String lastName,
      String mobile, bool? auto_approve_restaurant) async {
    try {
  ...
  static Future<auth.UserCredential?> reAuthUser(AuthProviders provider,
      {String? email, String? password, String? smsCode, String? verificationId, AccessToken? accessToken, apple
          .AuthorizationResult? appleCredential}) async {
