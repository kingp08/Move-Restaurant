...
import '../ordersScreen/ScheduleOrdersScreen.dart';

enum DrawerSelection { Orders, ScheduleOrders, DineIn, DineInReq, ManageProducts, AddRestauarnt, createTable, addStory, Offers, SpecialOffer, inbox,WorkingHours, Profile, Wallet, BankInfo, termsCondition, privacyPolicy,chooseLanguage, Logout }
...
class _ContainerScreen extends State<ContainerScreen> {
  ...
  void initState() {
    ...
    FireStoreUtils.getCurrentUser(MyAppState.currentUser == null ? widget.userId! : MyAppState.currentUser!.userID).then((value) {
      setState(() {
        ...
        print("changeOrderStatus ${MyAppState.currentUser}");
      });
    });
  ...
  getSpecialDiscount() async {
    ...
    prefs.setString("vendorID", MyAppState.currentUser!.vendorID);
  }
  ...
  void toggle() async {
    await pr?.show();
    await FireStoreUtils.updatestatus(vendorModel!, !isOn);
    await pr?.hide();
    setState(() => isOn = !isOn);
  }
        ...
        child: ListView(
          ...
          children: [
            ...
            ListTileTheme(
              ...
              selectedColor: Color(COLOR_PRIMARY),
              child: ListTile(
                selected: _drawerSelection == DrawerSelection.ScheduleOrders,
                title: Text('Schedule Orders').tr(),
                onTap: () {
                  Navigator.pop(context);
                  setState(
                        () {
                      _drawerSelection = DrawerSelection.ScheduleOrders;
                      _appBarTitle = 'Schedule Orders'.tr();
                      _currentWidget = ScheduleOrdersScreen();
                    },
                  );
                },
                leading: Image.asset(
                  'assets/images/app_logo.png',
                  color: _drawerSelection == DrawerSelection.ScheduleOrders
                      ? Color(COLOR_PRIMARY)
                      : isDarkMode(context)
                      ? Colors.grey.shade200
                      : Colors.grey.shade600,
